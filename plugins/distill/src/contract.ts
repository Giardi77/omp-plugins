...
export const PROPOSE_LESSONS_DESCRIPTION = [
  ...
  `  skill's SKILL.md gains the line pointing at it; the skill stays the entry point. A reference that`,
  `  is already there is trimmed by quoting its lines (the skill already points at it, so only the`,
  `  reference is written), and one that is not cannot be trimmed — there is nothing in it to remove.`,
  ...
  `AGENTS.md is not a target, and neither is the host's own configuration: when the lesson belongs in`,
  `one of those files, name the file and why in \`rationale\` instead of proposing it.`,
  ``,
